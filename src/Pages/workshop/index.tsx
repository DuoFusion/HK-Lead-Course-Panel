...
const WorkshopContainer = () => {
  const { pageNumber, pageSize , isActive, params, handleSetSearch, handlePaginationChange, setActive } = useBasicTableFilterHelper({
  ...
  const columns: ColumnsType<WorkshopType> = [
    ...
    },
    {
      ...
      dataIndex: "date",
      key: "date",
      render: (date: string) => (FormatDate(date) ? <Tag color="geekblue">{FormatDate(date)}</Tag> : "-"),
    },
    {
      title: "time",
      dataIndex: "time",
      key: "time",
      render: (time: string) => (FormatTime(time) ? <Tag color="green">{FormatTime(time)}</Tag> : "-"),
    },
    {
      title: "duration",
      dataIndex: "duration",
  ...
  return (
    <Fragment>
      ...
      <Container fluid className="custom-table">
        <CardWrapper Search={(e) => handleSetSearch(e)} searchClass="col-md-8" isActiveFilter={isActive} onActiveFilterChange={setActive} btnTitle="Add Workshop" btnClick={() => navigate(ROUTES.WORKSHOP.ADD_EDIT_WORKSHOP)}>
          <Table
